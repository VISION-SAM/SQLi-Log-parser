...
2. Ensure you have **Python 3.6 or higher** installed:
   ```bash
   python --version
No external dependencies are required. The script uses standard internal Python engine modules.🕹️ Usage GuideCommand-Line ArgumentsFlagLong FlagRequiredDescription-f--fileYesPath to the target web access log file to analyze.-h--helpNoDisplays the automated help menu overview.Running the AnalyzerTo execute a scan against a local log file, open your terminal and run:Bashpython log_analyzer.py -f access.log
...
[+] Source Malicious IPs Blocklist:
  └── 192.168.1.45
  └── 10.0.4.11
🛡️ Security DisclaimerThis utility is designed strictly for educational, defensive, and authorized incident response security workflows. Always ensure you have explicit permission before processing proprietary corporate application logs.
